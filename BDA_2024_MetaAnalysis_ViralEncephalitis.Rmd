...
# There are 8 contrasts and we want genes that are available in at least 7 contrasts
...
# Meta-analysis with 1 NA
MetaAnalysis_Results_1NA <- suppressWarnings(
...
# 1 NA
metaOutput_1NA <- MetaAnalysis_Results_1NA[[1]]
MetaAnalysis_Annotation_1NA <- MetaAnalysis_Results_1NA[[2]]
...
### 14) Create forest plot for statistically significant genes 
...
MakeForestPlots <- function(metaOutputFDR_annotated, GeneSymbol, species){
   if (species == "Mouse") {
  ...
  fc <- as.numeric(gene_row$Log2Fc_estimate)
...
for(i in c(1:length(differentials))){
    print(sum(is.na(differentials[[i]][,8])))
}


apply(Mouse_MetaAnalysis_FoldChanges, 1, function(y) sum(is.na(y)))
```
