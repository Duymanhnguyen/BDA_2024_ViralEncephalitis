...
- GSEA_Log2FC_1_0NA_KEGG

- GSEA_Log2FC_1_0NA_GO

- GSEA_Log2FC_1_0NA_Reactome

**Step 17:** Gene Set Enrichment Analysis with Brain.GMT database (developed by
Dr. Hagenauer)

*Output Object:*

- BrainGMT_metaOutputFDR_1_0NA

- BrainGMT_metaOutputFDR_1_0NA_cleaned

- BrainGMT_metaOutputFDR_OrderedByLog2FC_1_0NA

- BrainGMT_Mouse

- GSEA_Log2FC_1_0NA_BrainGMT
...
EntrezGeneList_Log2FC_1_0NA <- metaOutputFDR_OrderbyPval_Log2FC_1_0NA %>%
  filter(FDR < 0.05) %>%
  pull(Mouse_EntrezGene.ID)

# Extract gene entrez ID for genes with FDR < 0.05 & |Log2FC| > 2
EntrezGeneList_Log2FC_2_0NA <- metaOutputFDR_OrderbyPval_Log2FC_2_0NA %>%
  filter(FDR < 0.05) %>%
  pull(Mouse_EntrezGene.ID)
...
### 16) Gene Set Enrichment Analysis (GSEA) with KEGG/GO/Reactome databases
...
GSEAPerform_KEGG_GO_Reactome <- function(EntrezGeneList,
                                         pvalueCutOff,
                                         qvalueCutOff){
  # Load the mouse gene annotation database 
  OrgDb <- org.Mm.eg.db
...
# Function use
GSEA_Log2FC_1_0NA <- GSEAPerform_KEGG_GO_Reactome(EntrezGeneList_Log2FC_1_0NA,
                                                  0.5,
                                                  0.5)

GSEA_Log2FC_2_0NA <- GSEAPerform_KEGG_GO_Reactome(EntrezGeneList_Log2FC_2_0NA,
                                                  0.5,
                                                  0.5)

# Visualize the GSEA results
# png("GSEA_Log2FC_1_0NA_KEGG.png", 8, 5, "in", res = 300)
dotplot(GSEA_Log2FC_1_0NA$KEGG, showCategory = 10, font.size = 7)
# dev.off()

# png("GSEA_Log2FC_2_0NA_KEGG.png", 8, 5, "in", res = 300)
dotplot(GSEA_Log2FC_2_0NA$KEGG, showCategory = 10, font.size = 7)
# dev.off()
...
### 17) Gene Set Enrichment Analysis (GSEA) with Brain.GMT database
...
BrainGMT_metaOutputFDR_1_0NA_FDR_0.05 <- metaOutputFDR_OrderbyPval_Log2FC_1_0NA %>%
  filter(FDR < 0.05)

BrainGMT_metaOutputFDR_1_0NA <- setNames(
  BrainGMT_metaOutputFDR_1_0NA_FDR_0.05$Log2FC_estimate,
  BrainGMT_metaOutputFDR_1_0NA_FDR_0.05$Mouse_Symbol
)

# Remove genes with NA or duplicated names
BrainGMT_metaOutputFDR_1_0NA_cleaned <- BrainGMT_metaOutputFDR_1_0NA %>%
...
BrainGMT_metaOutputFDR_OrderedByLog2FC_1_0NA <- BrainGMT_metaOutputFDR_1_0NA_cleaned[order(BrainGMT_metaOutputFDR_1_0NA_cleaned)]
...
BrainGMT_metaOutputFDR_2_0NA_FDR_0.05 <- metaOutputFDR_OrderbyPval_Log2FC_2_0NA %>%
  filter(FDR < 0.05)

BrainGMT_metaOutputFDR_2_0NA <- setNames(
  BrainGMT_metaOutputFDR_2_0NA_FDR_0.05$Log2FC_estimate,
  BrainGMT_metaOutputFDR_2_0NA_FDR_0.05$Mouse_Symbol
)

# Remove genes with NA or duplicated names
BrainGMT_metaOutputFDR_2_0NA_cleaned <- BrainGMT_metaOutputFDR_2_0NA %>%
  # Convert to dataframe for easier manipulation
  enframe(name = "gene_name", value = "Log2FC_estimate") %>%
  # Remove rows with NA names
  filter(!is.na(gene_name)) %>%
...
# Order the vector by Log2FC in ascending order
BrainGMT_metaOutputFDR_OrderedByLog2FC_2_0NA <- BrainGMT_metaOutputFDR_2_0NA_cleaned[order(BrainGMT_metaOutputFDR_2_0NA_cleaned)]

# Perform fast fGSEA with Brain.GMT
GSEA_Log2FC_2_0NA_BrainGMT <- fgseaSimple(BrainGMT_Mouse, 
                                          BrainGMT_metaOutputFDR_OrderedByLog2FC_2_0NA,
                                          # Number of permutations for the GSEA
                                          nperm = 10000,
                                          # Minimum size of gene sets to consider
                                          minSize = 10,
