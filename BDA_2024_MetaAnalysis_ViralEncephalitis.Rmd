...
**Step 16:** Gene Set Enrichment Analysis with KEGG/GO/Reactome databases
...
### 16) Gene Set Enrichment Analysis (GSEA) with KEGG/GO/Reactome databases [Need to fix]

The difference between GSEA and fGSEA:
...
- *Goal:* The goal of GSEA is to identify whether specific gene sets are overrrepresented/
enriched in a ranked list of genes, which is often derived from differential expression
analysis.

- *Method:* GSEA computes enrichment scores by comparing the observed gene set against 
the background, using permutation-based statistical methods to evaluate significance.
When the entire gene sets are evaluated, the estimated significance is adjusted to
account for multiple hypothesis testing.

- *Sensitivity:* The results can be sensitive to the choice of the background gene 
set. An incorrect or incomplete background may lead to misleading conclusions.

**2.Fast Gene Set Enrichment Analysis (fGSEA)**
...
which simplifies the process but rely on a pre-ranked list of gene, either by p-values
or log2FC.
...
Ref for GSEA with GO database using `clusterProfiler`: https://yulab-smu.top/biomedical-knowledge-mining-book/clusterprofiler-go.html

Ref for GSEA with KEGG database using `clusterProfiler`:
...
```{r func_GSEA_KEGG_GO_Reactome}
# Create a function to perform GSEA on common databases, including KEGG, GO, and 
# Reactome
GSEAPerform_KEGG_GO_Reactome <- function(EntrezGeneList,
                                         metaOuput,
                                         pvalueCutOff,
                                         qvalueCutOff){
  
  # Perform GSEA with KEGG database 
  KEGG_GSEA_result <- enrichKEGG(gene = EntrezGeneList,
  ...
  GO_GSEA_result <- enrichGO(gene = EntrezGeneList,
  ...
  # Perform GSEA with Reactome database 
  Reactome_GSEA_result <- enrichPathway(gene = EntrezGeneList,
                                        # Organism code for Mus musculus
                                        organism = "mouse",
                                        # Address the background gene set (output of meta-analysis)
                                        universe = metaOuput,
  ...
  return(list(KEGG = KEGG_GSEA_result, 
              GO = GO_GSEA_result,
              Reactome = Reactome_GSEA_result))
}
```


```{r run_GSEA_KEGG_GO_Reactome, cache = TRUE}
# Function use
EntrezGeneList_metaOutputFDR_0NA <- as.character(metaOutputFDR_Annotated_0NA$Mouse_EntrezGene.ID)

GSEA_Log2FC_1_0NA <- GSEAPerform_KEGG_GO_Reactome(EntrezGeneList_OrderbyPval_Log2FC_1_0NA,
                                                  EntrezGeneList_metaOutputFDR_0NA,
                                                  0.05,
                                                  0.05)

GSEA_Log2FC_2_0NA <- GSEAPerform_KEGG_GO_Reactome(EntrezGeneList_OrderbyPval_Log2FC_2_0NA,
                                                  EntrezGeneList_metaOutputFDR_0NA,
                                                  0.05,
                                                  0.05)
...
# Visualize the GSEA results with KEGG database 

# Top 10 pathways using barplot presentation for genes with |Log2FC| > 1 and FDR < 0.05

# png("GSEA_Log2FC_1_0NA_KEGG.png", 8, 5, "in", res = 300)
barplot(GSEA_Log2FC_1_0NA$KEGG, showCategory = 10, font.size = 7)
# dev.off()


# Top 10 pathways using barplot presentation for genes with |Log2FC| > 2 and FDR < 0.05

# png("GSEA_Log2FC_2_0NA_KEGG.png", 8, 5, "in", res = 300)
barplot(GSEA_Log2FC_2_0NA$KEGG, showCategory = 10, font.size = 7)
# dev.off()
...
# Visualize the GSEA results with GO database 

# Visualize the results with directed acyclic graph

# png("GSEA_Log2FC_1_0NA_GO_Network.png", 8, 5, "in", res = 300)
goplot(GSEA_Log2FC_1_0NA$GO)
# dev.off()

# png("GSEA_Log2FC_2_0NA_GO_Network.png", 8, 5, "in", res = 300)
goplot(GSEA_Log2FC_2_0NA$GO)
# dev.off()

# Top 10 pathways using barplot presentation for genes with |Log2FC| > 1 and FDR < 0.05

# png("GSEA_Log2FC_1_0NA_GO.png", 8, 5, "in", res = 300)
barplot(GSEA_Log2FC_1_0NA$GO, showCategory = 10, font.size = 7)
# dev.off()

# Top 10 pathways using barplot presentation for genes with |Log2FC| > 2 and FDR < 0.05

# png("GSEA_Log2FC_2_0NA_GO.png", 8, 5, "in", res = 300)
barplot(GSEA_Log2FC_2_0NA$GO, showCategory = 10, font.size = 7)
# dev.off()
```
...
# png("GSEA_Log2FC_1_0NA_Reactome.png", 8, 5, "in", res = 300)
barplot(GSEA_Log2FC_1_0NA$Reactome, showCategory = 10, font.size = 7)
# dev.off()

# Top 10 pathways using barplot presentation for genes with |Log2FC| > 2 and FDR < 0.05

# png("GSEA_Log2FC_2_0NA_Reactome.png", 8, 5, "in", res = 300)
barplot(GSEA_Log2FC_2_0NA$Reactome, showCategory = 10, font.size = 7)
# dev.off()
```
...
# directory
GSEA_Log2FC_1_0NA_KEGG <- as.data.frame(GSEA_Log2FC_1_0NA$KEGG)
row.names(GSEA_Log2FC_1_0NA_KEGG) <- NULL
write.csv(GSEA_Log2FC_1_0NA_KEGG, "GSEA_Log2FC_1_0NA_KEGG.csv")

GSEA_Log2FC_2_0NA_KEGG <- as.data.frame(GSEA_Log2FC_2_0NA$KEGG)
row.names(GSEA_Log2FC_2_0NA_KEGG) <- NULL
write.csv(GSEA_Log2FC_2_0NA_KEGG, "GSEA_Log2FC_2_0NA_KEGG.csv")

GSEA_Log2FC_1_0NA_GO <- as.data.frame(GSEA_Log2FC_1_0NA$GO)
row.names(GSEA_Log2FC_1_0NA_GO) <- NULL
write.csv(GSEA_Log2FC_1_0NA_GO, "GSEA_Log2FC_1_0NA_GO.csv")

GSEA_Log2FC_2_0NA_GO <- as.data.frame(GSEA_Log2FC_2_0NA$GO)
row.names(GSEA_Log2FC_2_0NA_GO) <- NULL
write.csv(GSEA_Log2FC_2_0NA_GO, "GSEA_Log2FC_2_0NA_GO.csv")

GSEA_Log2FC_1_0NA_Reactome <- as.data.frame(GSEA_Log2FC_1_0NA$Reactome)
row.names(GSEA_Log2FC_1_0NA_Reactome) <- NULL
write.csv(GSEA_Log2FC_1_0NA_Reactome, "GSEA_Log2FC_1_0NA_Reactome.csv")

GSEA_Log2FC_2_0NA_Reactome <- as.data.frame(GSEA_Log2FC_2_0NA$Reactome)
row.names(GSEA_Log2FC_2_0NA_Reactome) <- NULL
write.csv(GSEA_Log2FC_2_0NA_Reactome, "GSEA_Log2FC_2_0NA_Reactome.csv")
```
...
each curated to be relevant to brain functions, brain celltypes, co-expression networks, 
...
```{r run_GSEA_BrainGMT_1, cache = TRUE}
...
BrainGMTgenedata_metaOutputFDR_Log2FC_1_0NA <- setNames(
  metaOutputFDR_OrderbyPval_Log2FC_1_0NA$Log2FC_estimate,
  metaOutputFDR_OrderbyPval_Log2FC_1_0NA$Mouse_Symbol
)

# Remove genes with NA or duplicated names
BrainGMTgenedata_metaOutputFDR_Log2FC_1_0NA_cleaned <- BrainGMTgenedata_metaOutputFDR_Log2FC_1_0NA %>%
...
BrainGMTgenedata_metaOutputFDR_OrderedByLog2FC_1_0NA <- BrainGMTgenedata_metaOutputFDR_Log2FC_1_0NA_cleaned[order(BrainGMTgenedata_metaOutputFDR_Log2FC_1_0NA_cleaned)]

# Import the Brain.GMT for the specific species of interest
# Ignore the warning about incomplete line 
BrainGMT_Mouse <- gmtPathways("BrainGMTv2_MouseOrthologs.gmt.txt")

# Perform fast fGSEA with Brain.GMT
GSEA_Log2FC_1_0NA_BrainGMT <- fgseaSimple(BrainGMT_Mouse, 
                                          BrainGMTgenedata_metaOutputFDR_OrderedByLog2FC_1_0NA,
...
# the GSEA results to a comma-separated string
GSEA_Log2FC_1_0NA_BrainGMT$leadingEdge <- vapply(GSEA_Log2FC_1_0NA_BrainGMT$leadingEdge, 
                                                 paste, 
                                                 collapse= ",", 
                                                 character(1L))

# Export the matched pathways into the working directory
write.csv(GSEA_Log2FC_1_0NA_BrainGMT, "GSEA_Log2FC_1_0NA_BrainGMT.csv")

# It is not sorted by p-value?
# padj: FDR corrected p-value (using familywise-error rate???)
# ES & NES: Enrichment Score and Normalized Enrichment Score for each gene set
# Positive ES & NES means that the gene set is enriched with upregulation in response
# to your variable of interest
# Negative ES & NES means that the gene set is enriched with downregulation in response
# to your variable of interest
# leadingEdge: The core of a gene set that accounts for the enrichment signal
# nMoreExtreme???
# Size???
```


```{r run_GSEA_BrainGMT_2, cache = TRUE}
# fGSEA uses the entirety of the gene list but need to pre-ranked with either p-values
# or Log2FC. For Brain.GMT, we use gene list pre-ranked by Log2FC. 
...
BrainGMTgenedata_metaOutputFDR_Log2FC_2_0NA <- setNames(
  metaOutputFDR_OrderbyPval_Log2FC_2_0NA$Log2FC_estimate,
  metaOutputFDR_OrderbyPval_Log2FC_2_0NA$Mouse_Symbol
)

# Remove genes with NA or duplicated names
BrainGMTgenedata_metaOutputFDR_log2FC_2_0NA_cleaned <- BrainGMTgenedata_metaOutputFDR_Log2FC_2_0NA %>%
  # Convert to dataframe for easier manipulation
  enframe(name = "gene_name", value = "Log2FC_estimate") %>%
  # Remove rows with NA names
  filter(!is.na(gene_name)) %>%
...
# Order the vector by Log2FC in ascending order
BrainGMTgenedata_metaOutputFDR_OrderedByLog2FC_2_0NA <- BrainGMTgenedata_metaOutputFDR_log2FC_2_0NA_cleaned[order(BrainGMTgenedata_metaOutputFDR_log2FC_2_0NA_cleaned)]

# Perform fast fGSEA with Brain.GMT
GSEA_Log2FC_2_0NA_BrainGMT <- fgseaSimple(BrainGMT_Mouse, 
                                          BrainGMTgenedata_metaOutputFDR_OrderedByLog2FC_2_0NA,
                                          # Number of permutations for the GSEA
                                          nperm = 10000,
                                          # Minimum size of gene sets to consider
                                          minSize = 10,
...
# Converts the list of leading edge genes (genes contributing to enrichment) in 
# the GSEA results to a comma-separated string
GSEA_Log2FC_2_0NA_BrainGMT$leadingEdge <- vapply(GSEA_Log2FC_2_0NA_BrainGMT$leadingEdge, 
                                                 paste, 
                                                 collapse= ",", 
                                                 character(1L))

# Export the matched pathways into the working directory
write.csv(GSEA_Log2FC_2_0NA_BrainGMT, "GSEA_Log2FC_2_0NA_BrainGMT.csv")
```
